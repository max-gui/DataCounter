...
var nonWorkingWeekdays = {
	...
	2016 : [[1, 1], [2, 8], [2, 9], [2, 10], [2, 11], [2, 12], [4, 4], [5, 2], [6, 9], [6, 10]]
};
var workingWeekEnds = {
	2014 : [[9, 28], [10, 11]],
...
function listProcess(table, taxiData, dinnerData) {
	//var tbl = "<table><tr><th>打车</th><th>晚餐</th></tr><tr><td><tr>" + taxiData.join("</tr><tr>") + "</tr></td><td><tr>" + dinnerData.join("</tr><tr>") + "</tr></td></tr></table>";
...
function processTable(table) {
	...
	var tableMonth = getTableMonth(table);
	var maxDate;
	if (month == tableMonth) {
	...
	table.children('tbody').children('tr').each(function(i, row) {
		...
		$(row).children('td').each(function(j, cell) {
			...
			if (hasClocked) {
				...
				var colorTmp = isWorkingDay ? '#ed1941': '#007947';
				'#ed1941'$(listBodies[1]).children('td:first-child').css('background-color', colorTmp);
			}
	...
	var message = "本月已过" + totalWorkingDays + "个工作日，应工作时间" + getTimeText(targetWorkingMins) + '，实际工作时间' + getTimeText(totalClockedMins);
	if (targetWorkingMins < totalClockedMins) {
		message += "，已多工作了<span style='background: green; color: white;'>" + getTimeText(totalClockedMins - targetWorkingMins) + "</span>";
	} else if (targetWorkingMins > totalClockedMins) {
		message += "，已少工作了<span style='background: red; color: white;'>" + getTimeText(targetWorkingMins - totalClockedMins) + "</span>";
	}
	message += "。";
	table.parent().prepend(message);
}

function getTableMonth(table) {
	var text = table.find('table').find('td').text().trim();
	var yearIndex = text.indexOf('年');
	var monthIndex = text.indexOf('月');
	return parseInt(text.substring(yearIndex + 1, monthIndex));
}
...
function getMins(text, isWorkingDay) {
	...
	if (text.length < 11) {
		retValue.minis = 0;
		retValue.texi = 0;
		retValue.texiTime = ""retValue.tooEarly = false;
		retValue.dinner = 0;
	...
	if (outHour < 9) {
		retValue.minis = 0;
		retValue.texi = 0;
		retValue.texiTime = ""retValue.tooEarly = true;
		retValue.dinner = 0;
	...
	if (isWorkingDay) {
		...
		//    minAdjustment = 0;
		//} else if (inHour == 12) {
		//    minAdjustment = outHour > 12 ? 60 - inMinute : 0;
		//} else {
		//    if (outHour < 12) {
		//        minAdjustment = 0;
		//    } else if (outHour == 12) {
		//        minAdjustment = outMinute;
		//    }
		//}
	}
	var retValue = new Object();
	...
	retValue.dinner = dinnerEnale;
	retValue.preTexiTime = "02:" + Math.ceil(Math.random() * 40).toString();
	return retValue;
}

function isWorkingWeekEnd(year, month, day) {
	if (!workingWeekEnds[year]) {
		return false;
	}
	var isWorkingDay = false;
	$.each(workingWeekEnds[year],
	function(i, date) {
...
function isWorkingWeekDay(year, month, day) {
	if (!nonWorkingWeekdays[year]) {
		return true;
	}
	var isWorkingDay = true;
	$.each(nonWorkingWeekdays[year],
	function(i, date) {
		if (date[0] == month && date[1] == day) {
			isWorkingDay = false;
			return false;
